...
import { generateProjectCode, analyzeSeo, suggestAnalytics } from '../../services/geminiService';
...
const WebBuilder: React.FC = () => {
    ...
    const handleAnalysis = async (type: 'seo' | 'analytics') => {
        if (!fullHtmlPreview) return;
        setIsAnalyzing(true);
        setAnalysisResult(null);
        setError('');
        try {
            const response = type === 'seo' 
                ? await analyzeSeo(fullHtmlPreview) 
                : await suggestAnalytics(fullHtmlPreview);
            setAnalysisResult({
                title: type === 'seo' ? 'SEO Analysis Report' : 'Analytics Plan Suggestion',
    ...
    const getLanguageForCodeBlock = (filename: string): string => {
     ...
     const FeaturePlaceholder = ({ title, description, onClick, disabled }: { title: string, description: string, onClick?: () => void, disabled?: boolean }) => (
        <button onClick={onClick} disabled={disabled} className="p-2 text-left bg-slate-100 dark:bg-slate-800 rounded-md transition-colors hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed w-full">
            <p className="font-semibold text-xs text-slate-700 dark:text-slate-300">{title}</p>
    ...
    const renderControls = () => {
            ...
            return (
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {TEMPLATES.map(template => (
                        <button key={template.name} onClick={() => handleSelectTemplate(template)} className="p-3 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg text-left transition-colors border border-slate-200 dark:border-slate-700">
                            <p className="font-semibold text-sm text-brand-primary">{template.name}</p>
                            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{template.type} / {template.platform}</p>
                        </button>
                    ))}
                </div>
            );
        ...
        return (
            <>
                ...
                <div><label className="block text-xs font-medium mb-1">Project Name</label><input type="text" value={projectName} onChange={e => setProjectName(e.target.value)} placeholder="e.g., My Awesome Game" className="w-full text-sm bg-white dark:bg-slate-700/50 border border-slate-300 dark:border-slate-600 p-2 rounded-md"/></div>
                <div><label className="block text-xs font-medium mb-1">Copywriting Language</label><select value={language} onChange={e => setLanguage(e.target.value)} className="w-full text-sm bg-white dark:bg-slate-700/50 border border-slate-300 dark:border-slate-600 p-2 rounded-md">{LANGUAGES.map(l => <option key={l}>{l}</option>)}</select></div>
                <div><label className="block text-xs font-medium mb-1">Describe your Project</label><textarea rows={5} value={description} onChange={(e) => setDescription(e.target.value)} className="w-full text-sm bg-white dark:bg-slate-700/50 border border-slate-300 dark:border-slate-600 p-2 rounded-md" placeholder="Be detailed! e.g., A 2D platformer game where a squirrel collects nuts..." /></div>
                <button onClick={() => callApi(false)} disabled={isLoading || !description.trim() || !projectName.trim()} className="w-full py-2 bg-brand-primary text-white rounded-md font-semibold disabled:bg-slate-400 dark:disabled:bg-slate-600 flex justify-center">{isLoading ? <Spinner/> : 'Generate Project'}</button>
                {codeFiles && (
                    <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                        <label className="block text-sm font-medium mb-1">Refine & Customize</label>
                        <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">Describe a change, like a no-code editor.</p>
                        <div className="flex gap-2">
                            <input value={refinePrompt} onChange={e => setRefinePrompt(e.target.value)} onKeyDown={e => e.key === 'Enter' && callApi(true)} placeholder="e.g., Change the color scheme to blue" className="flex-grow text-sm bg-white dark:bg-slate-700/50 border border-slate-300 dark:border-slate-600 p-2 rounded-md" />
                            <button onClick={() => callApi(true)} disabled={isRefining || !refinePrompt} className="py-2 px-3 bg-brand-secondary text-white text-sm rounded-md disabled:bg-slate-400 dark:disabled:bg-slate-600 flex justify-center">{isRefining ? <Spinner/> : "Refine"}</button>
                        </div>
                    </div>
                )}
            </>
    ...
    return (
        <div className="p-6 h-full flex flex-col">
            {analysisResult && (
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setAnalysisResult(null)}>
                    <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-2xl max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
                        <header className="p-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center">
                            ...
                            <button onClick={() => setAnalysisResult(null)}>&times;</button>
                        </header>
                        <main className="p-6 overflow-y-auto prose prose-sm dark:prose-invert max-w-none">
                            <pre className="font-sans whitespace-pre-wrap">{analysisResult.content}</pre>
                        </main>
                    </div>
                </div>
            )}
            ...
            <div className="flex-grow grid md:grid-cols-3 gap-6 overflow-hidden">
                <div className="md:col-span-1 flex flex-col gap-3 overflow-y-auto pr-2">
                    <div className="flex bg-slate-200 dark:bg-slate-800 rounded-md p-1"><button onClick={() => setMode('template')} className={`flex-1 text-sm p-2 rounded ${mode === 'template' ? 'bg-brand-primary text-white' : 'hover:bg-slate-300 dark:hover:bg-slate-700'}`}>Start from Template</button><button onClick={() => setMode('custom')} className={`flex-1 text-sm p-2 rounded ${mode === 'custom' ? 'bg-brand-primary text-white' : 'hover:bg-slate-300 dark:hover:bg-slate-700'}`}>Custom Project</button></div>
                    {renderControls()}
                    {error && <p className="text-red-500 dark:text-red-400 text-sm mt-2">{error}</p>}
                     <div className="mt-auto pt-4 space-y-2">
                         ...
                         <div className="grid grid-cols-2 gap-2">
                            ...
                            <FeaturePlaceholder title="Sales Funnels" description="Create conversion paths." disabled />
                         </div>
                    </div>
                </div>

                <div className="md:col-span-2 bg-slate-100 dark:bg-slate-900/50 rounded-lg border border-slate-200 dark:border-slate-700 flex flex-col overflow-hidden">
                    ...
                    {codeFiles && (
                        ...
                        <div className="h-1/2 flex flex-col">
                           <div className="flex-shrink-0 p-2 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
                             <h4 className="text-sm font-semibold">Live Preview</h4>
                             <div className="flex items-center gap-2">
                                <button disabled className="text-xs py-1 px-3 bg-slate-300 dark:bg-slate-600 text-slate-500 dark:text-slate-400 rounded-md cursor-not-allowed">Export to WordPress</button>
                                <button disabled className="text-xs py-1 px-3 bg-slate-300 dark:bg-slate-600 text-slate-500 dark:text-slate-400 rounded-md cursor-not-allowed">Download Project (.zip)</button>
                             </div>
                           </div>
                           <iframe srcDoc={fullHtmlPreview} title="Website Preview" className="w-full h-full bg-white" sandbox="allow-scripts" />
                        </div>
                         <div className="h-1/2 flex flex-col border-t-2 border-slate-200 dark:border-slate-700">
                             <div className="flex-shrink-0 border-b border-slate-200 dark:border-slate-700"><div className="flex space-x-2 p-2 overflow-x-auto">{Object.keys(codeFiles).map(filename => (<button key={filename} onClick={() => setActiveFile(filename)} className={`text-xs px-3 py-1 rounded-md ${activeFile === filename ? 'bg-brand-primary text-white' : 'bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600'}`}>{filename}</button>))}</div></div>
