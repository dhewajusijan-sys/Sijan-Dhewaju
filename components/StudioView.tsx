...
const StudioView: React.FC<StudioViewProps> = ({ initialToolData, onDataConsumed, activeToolId, setActiveToolId }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row h-full">
      {/* Mobile Dropdown */}
      <div className="p-4 border-b border-brand-cyan/20 md:hidden">
        <label htmlFor="studio-tool-select" className="block text-sm font-medium text-slate-400 mb-1">
          Select a Tool
        </label>
        <div className="relative">
            <select
              ...
              className="block appearance-none w-full bg-charcoal-800/50 border border-blue-500/20 px-3 py-2 pr-8 rounded-md shadow-sm leading-tight focus:outline-none focus:ring-2 focus:ring-brand-cyan"
            >
      ...
      {/* Desktop Sidebar */}
      <aside className="hidden md:block w-64 flex-shrink-0 bg-charcoal-900/30 border-r border-brand-cyan/20 p-4 overflow-y-auto">
        <h2 className="text-lg font-bold mb-4 text-slate-100" style={{ textShadow: '0 0 8px rgba(34, 211, 238, 0.5)' }}>Studio Tools</h2>
        <nav className="flex flex-col gap-2">
          {STUDIO_TOOLS.map(tool => (
            ...
            >
              <div className="flex-shrink-0 w-6 h-6 text-slate-300">{tool.icon}</div>
              <div>
                <p className="font-semibold text-sm">{tool.name}</p>
                <p className="text-xs text-slate-400">{tool.description}</p>
              </div>
               {tool.comingSoon && <span className="ml-auto text-xs bg-charcoal-700 text-slate-300 px-2 py-0.5 rounded-full">Soon</span>}
            </button>
          ))}
        </nav>
      </aside>
